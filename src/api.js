...
export const getEvents = async () => {
    ...
    if (!accessToken || tokenCheck.error) {
      ...
      if (!code) {
        const response = await fetch(
          "https://8m8pi7mn1j.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
        );
        const result = await response.json();
  ...
  const getToken = async (code) => {
    ...
    const response = await fetch(
      'https://8m8pi7mn1j.execute-api.eu-central-1.amazonaws.com/dev/api/token' + '/' + encodeCode
    );
